...
func (s SelectStatement) Walk(w *ast.Writer) {
	w.NoIndent()
	w.Write("SELECT")
	w.NewLine()
	w.Indent()
...
func (s SelectExistsStatement) Walk(w *ast.Writer) {
	w.NoIndent()
	w.Write("SELECT ")
	if s.Not {
		w.Write("NOT ")
...
func (s SelectCountStatement) Walk(w *ast.Writer) {
	w.NoIndent()
...
func (l LimitClause) Walk(w *ast.Writer) {
	if l.Value == nil {
		return
	}
	w.NewLine()
...
func (i LimitInt) Walk(w *ast.Writer) {
	w.Write(strconv.FormatInt(int64(i), 10))
}

type LimitUint uint64

func (u LimitUint) Walk(w *ast.Writer) {
...
func (l OffsetClause) Walk(w *ast.Writer) {
	if l.Value == nil {
		return
	}
	w.NewLine()
...
func (i OffsetInt) Walk(w *ast.Writer) {
	w.Write(strconv.FormatInt(int64(i), 10))
}

type OffsetUint uint64

func (u OffsetUint) Walk(w *ast.Writer) {
	w.Write(strconv.FormatUint(uint64(u), 10))
}
...
func (c OrderClause) Walk(w *ast.Writer) {
	if len(c.List) == 0 {
		return
	}
	w.NewLine()
...
func (o OrderBy) Walk(w *ast.Writer) {
	o.Column.Walk(w)
	if o.Desc {
		w.Write(" DESC")
	} else {
...
func (c JoinClause) Walk(w *ast.Writer) {
	if len(c.List) == 0 {
		return
	}
