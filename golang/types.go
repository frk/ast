...
func (at ArrayType) Walk(w *ast.Writer) {
	w.Write("[")
	at.Len.Walk(w)
	w.Write("]")
	at.Elem.Walk(w)
...
func (st SliceType) Walk(w *ast.Writer) {
	w.Write("[]")
	st.Elem.Walk(w)
}
...
func (st StructType) Walk(w *ast.Writer) {
	w.Write("struct")
	if st.Fields == nil {
		w.Write("{}")
		return
...
func (f Field) Walk(w *ast.Writer) {
	if f.Doc != nil {
		f.Doc.Walk(w)
		w.Write("\n")
	}
...
func (list FieldList) Walk(w *ast.Writer) {
	if len(list) < 1 {
		return
	}
...
func (ft FuncType) Walk(w *ast.Writer) {
	w.Write("func")
	Signature(ft).Walk(w)
}
...
func (sig Signature) Walk(w *ast.Writer) {
	w.Write("(")
	sig.Params.Walk(w)
	w.Write(")")
...
func (list ParamList) Walk(w *ast.Writer) {
	if len(list) < 1 {
		return
	}
...
func (p Param) Walk(w *ast.Writer) {
	if p.Names != nil {
		p.Names.Walk(w)
		w.Write(" ")
	}
...
func (it InterfaceType) Walk(w *ast.Writer) {
	w.Write("interface")
	if it.Methods == nil {
		w.Write("{}")
		return
...
func (m Method) Walk(w *ast.Writer) {
	m.Name.Walk(w)
	m.Type.Walk(w)
}
...
func (list MethodList) Walk(w *ast.Writer) {
	if len(list) < 1 {
		return
	}
...
func (mt MapType) Walk(w *ast.Writer) {
	w.Write("map[")
	mt.Key.Walk(w)
	w.Write("]")
	mt.Value.Walk(w)
...
func (ct ChanType) Walk(w *ast.Writer) {
	if ct.Dir == ChanRecv {
		w.Write("<-")
	}
	w.Write("chan")
...
func (pt PointerType) Walk(w *ast.Writer) {
	w.Write("*")
	pt.Elem.Walk(w)
}
