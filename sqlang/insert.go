...
func (s InsertStatement) Walk(w *ast.Writer) {
	w.NoIndent()
	s.Head.Walk(w)
...
func (h InsertHead) Walk(w *ast.Writer) {
	w.Write("INSERT INTO ")
	h.Table.Walk(w)
	w.Write(" ")
...
func (t InsertTail) Walk(w *ast.Writer) {
	if t.OnConflict != nil {
		t.OnConflict.Walk(w)
...
func (c OverridingClause) Walk(w *ast.Writer) {
	if len(c) < 1 {
		return
	}
	w.Write("OVERRIDING ")
...
func (s InsertSource) Walk(w *ast.Writer) {
...
func (vc ValuesClause) Walk(w *ast.Writer) {
	w.Write("VALUES ")
	if len(vc.Exprs) == 0 {
		return
	}
...
func (el ExprList) Walk(w *ast.Writer) {
	w.Write("(")
...
func (oc *OnConflictClause) Walk(w *ast.Writer) {
	if oc == nil {
		return
	}
...
func (cc ConflictColumns) Walk(w *ast.Writer) {
	w.Write("(")
	for i, c := range cc {
		if i > 0 {
			w.Write(", ")
...
func (ind ConflictIndex) Walk(w *ast.Writer) {
	w.Write("(" + ind.Expr + ")")
	if len(ind.Pred) > 0 {
...
func (cc ConflictConstraint) Walk(w *ast.Writer) {
...
func (a *ConflictAction) Walk(w *ast.Writer) {
	if a == nil {
		w.Write("DO NOTHING")
		return
	}
...
func (x UpdateExcluded) Walk(w *ast.Writer) {
	if len(x.Columns) > 0 {
		if !x.Compact {
			w.NewLine()
		} else {
...
func (rc ReturningClause) Walk(w *ast.Writer) {
	if len(rc) < 1 {
		return
	}
