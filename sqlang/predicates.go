...
func (t TRUTH) Walk(w *ast.Writer) {
	switch t {
	case UNKNOWN:
		w.Write("UNKNOWN")
	case TRUE:
...
func (p TruthPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
	if p.Not {
		w.Write(" IS NOT ")
	} else {
...
func (op CMPOP) Walk(w *ast.Writer) {
	switch op {
	case EQUAL:
		w.Write(" = ")
	case NOT_EQUAL:
...
func (p ComparisonPredicate) Walk(w *ast.Writer) {
	p.LPredicand.Walk(w)
	p.Cmp.Walk(w)
	p.RPredicand.Walk(w)
}
...
func (s SYMMETRY) Walk(w *ast.Writer) {
	switch s {
	case ASYMMETRIC:
		w.Write("ASYMMETRIC ")
	case SYMMETRIC:
...
func (p BetweenPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
...
func (p InPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
...
func (p LikePredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
...
func (p ILikePredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
...
func (p SimilarPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
...
func (op REGEXOP) Walk(w *ast.Writer) {
	switch op {
	case MATCH:
		w.Write(" ~ ")
	case MATCH_CI:
...
func (p RegexPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
	p.Op.Walk(w)
	p.Pattern.Walk(w)
}
...
func (p NullPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
	if p.Not {
		w.Write(" IS NOT NULL ")
	} else {
...
func (q QUANTIFIER) Walk(w *ast.Writer) {
	switch q {
	case ALL:
		w.Write("ALL")
	case ANY:
...
func (p ArrayComparisonPredicate) Walk(w *ast.Writer) {
	p.Predicand.Walk(w)
	p.Cmp.Walk(w)
	p.Qua.Walk(w)
	w.Write("(")
...
func (p DistinctPredicate) Walk(w *ast.Writer) {
	p.LPredicand.Walk(w)
	if p.Not {
...
func (EmptyEscapeCharacter) Walk(w *ast.Writer) {
	w.Write("''")
}

type CharacterValueExpr byte

func (x CharacterValueExpr) Walk(w *ast.Writer) {
	w.Write("'" + string(x) + "'")
}
