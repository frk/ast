...
func (x PointerIndirectionExpr) Walk(w *ast.Writer) {
	w.Write("*")
	x.X.Walk(w)
}
...
func (x UnaryExpr) Walk(w *ast.Writer) {
	w.Write(string(x.Op))
	x.X.Walk(w)
}
...
func (x SelectorExpr) Walk(w *ast.Writer) {
	x.X.Walk(w)
	w.Write(".")
	x.Sel.Walk(w)
}
...
func (x CallExpr) Walk(w *ast.Writer) {
	x.Fun.Walk(w)
	w.Write("(")
	x.Args.Walk(w)
	w.Write(")")
...
func (a ArgsList) Walk(w *ast.Writer) {
	if a.List != nil {
...
func (x CallNewExpr) Walk(w *ast.Writer) {
	w.Write("new(")
	x.Type.Walk(w)
	w.Write(")")
}
...
func (x CallMakeExpr) Walk(w *ast.Writer) {
	w.Write("make(")
	x.Type.Walk(w)
	if x.Size != nil {
		w.Write(", ")
...
func (x CallLenExpr) Walk(w *ast.Writer) {
	w.Write("len(")
	x.X.Walk(w)
	w.Write(")")
}
...
func (x ParenExpr) Walk(w *ast.Writer) {
	w.Write("(")
	if x.X != nil {
		x.X.Walk(w)
	}
...
func (x IndexExpr) Walk(w *ast.Writer) {
	x.X.Walk(w)
	w.Write("[")
	x.Index.Walk(w)
	w.Write("]")
...
func (x SliceExpr) Walk(w *ast.Writer) {
	x.X.Walk(w)
	w.Write("[")
	if x.Low != nil {
		x.Low.Walk(w)
...
func (x TypeAssertExpr) Walk(w *ast.Writer) {
	x.X.Walk(w)
	w.Write(".(")
	if x.Type != nil {
		x.Type.Walk(w)
...
func (xx MultiLineExpr) Walk(w *ast.Writer) {
