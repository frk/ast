...
func (s DeclStmt) Walk(w *ast.Writer) {
	s.Decl.Walk(w)
}
...
func (s LabeledStmt) Walk(w *ast.Writer) {
	s.Label.Walk(w)
	w.Write(":\n")
	s.Stmt.Walk(w)
}
...
func (s ExprStmt) Walk(w *ast.Writer) {
	s.X.Walk(w)
}
...
func (s SendStmt) Walk(w *ast.Writer) {
	s.Chan.Walk(w)
	w.Write(" <- ")
	s.Value.Walk(w)
}
...
func (s IncDecStmt) Walk(w *ast.Writer) {
	s.X.Walk(w)
...
func (s AssignStmt) Walk(w *ast.Writer) {
	s.Lhs.Walk(w)
	w.Write(" ")
	w.Write(string(s.Token))
	w.Write(" ")
...
func (s GoStmt) Walk(w *ast.Writer) {
	w.Write("go ")
	s.Call.Walk(w)
}
...
func (s DeferStmt) Walk(w *ast.Writer) {
	w.Write("defer ")
	s.Call.Walk(w)
}
...
func (s ReturnStmt) Walk(w *ast.Writer) {
	w.Write("return")
	if s.Result != nil {
		w.Write(" ")
		s.Result.Walk(w)
...
func (s BranchStmt) Walk(w *ast.Writer) {
	w.Write(string(s.Token))
...
func (s BlockStmt) Walk(w *ast.Writer) {
	w.Write("{")
...
func (s IfStmt) Walk(w *ast.Writer) {
	w.Write("if ")
	if s.Init != nil {
		s.Init.Walk(w)
		w.Write("; ")
...
func (s SwitchStmt) Walk(w *ast.Writer) {
	w.Write("switch ")
	if s.Init != nil {
		s.Init.Walk(w)
		w.Write("; ")
...
func (s CaseClause) Walk(w *ast.Writer) {
	if s.List != nil {
		w.Write("case ")
		s.List.Walk(w)
		w.Write(":")
...
func (s TypeSwitchStmt) Walk(w *ast.Writer) {
	w.Write("switch ")
	if s.Init != nil {
		s.Init.Walk(w)
		w.Write("; ")
...
func (g TypeSwitchGuard) Walk(w *ast.Writer) {
...
func (s TypeCaseClause) Walk(w *ast.Writer) {
	if s.List != nil {
		w.Write("case ")
		s.List.Walk(w)
		w.Write(":")
...
func (s SelectStmt) Walk(w *ast.Writer) {
	w.Write("select ")
	if len(s.Cases) == 0 {
		w.Write("{}")
		return
...
func (s CommClause) Walk(w *ast.Writer) {
	if s.Comm != nil {
		w.Write("case ")
		s.Comm.Walk(w)
		w.Write(":")
...
func (s ForStmt) Walk(w *ast.Writer) {
	w.Write("for ")
	if s.Clause != nil {
		s.Clause.Walk(w)
		w.Write(" ")
...
func (f ForCondition) Walk(w *ast.Writer) {
	if f.X != nil {
		f.X.Walk(w)
	}
}
...
func (f ForClause) Walk(w *ast.Writer) {
	if f.Init != nil {
		f.Init.Walk(w)
	}
	w.Write("; ")
...
func (f ForRangeClause) Walk(w *ast.Writer) {
	if f.Key != nil {
		f.Key.Walk(w)
	} else if f.Value != nil {
		w.Write("_")
